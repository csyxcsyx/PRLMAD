...
class FrontendSmokeTests(unittest.TestCase):
    ...
    def test_profile_steps_use_fixed_open_layout(self) -> None:
        ...
        self.assertRegex(source, r"\.profile-option\s*\{[^}]*border:\s*0 !important;")
        self.assertRegex(source, r"\.profile-option\.is-selected\s*\{[^}]*border-radius:\s*1rem !important;")
        self.assertIn("background: #efe9df !important", source)
        self.assertRegex(source, r"\.profile-custom-input\s*\{[^}]*border:\s*0 !important;")
        self.assertRegex(source, r"\.profile-custom-input\s*\{[^}]*border-radius:\s*\.8rem !important;")
        self.assertIn("background: #f5f0e8 !important", source)
        self.assertIn("background: #fbede7 !important", source)
        self.assertRegex(source, r"\.profile-custom-input:focus\s*\{[^}]*box-shadow:\s*none !important;")
